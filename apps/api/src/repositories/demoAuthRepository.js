...
export async function removePlanItem(userId,id){const index=plan.findIndex((x)=>x.userId===userId&&x.plan_item_id===id);if(index<0)return false;plan.splice(index,1);return true}
export async function listCorrections(){return corrections}
export async function updateCorrection(id,status){const item=corrections.find((x)=>x.id===id);if(item)item.status=status;return item||null}
