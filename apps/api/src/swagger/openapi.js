export const openapi = {
  ...
  components:{
    ...
    schemas:{
      ...
      AdmissionPossibilityRequest:{allOf:[{$ref:'#/components/schemas/EnemRequest'},{type:'object',properties:{q:{type:'string'},city:{type:'string'},state:{type:'string',minLength:2,maxLength:2},competitionModality:{type:'string'},year:{type:'integer'},round:{type:'string'},roundKind:{type:'string',enum:['regular','waiting_snapshot']},shift:{type:'string',enum:['matutino','vespertino','noturno','integral']},page:{type:'integer',minimum:1},limit:{type:'integer',minimum:1,maximum:100}}}]}
    },
  ...
  paths:{
    ...
    '/admission-history':{get:{tags:['Ingresso'],summary:'Lista a cobertura histórica oficial disponível sem receber notas pessoais',responses:{200:{description:'Cenários, fontes, datas e cobertura parcial'}}}},
    '/enem/possibilities':{post:{tags:['Ingresso'],summary:'Compara as cinco notas com pesos e mínimos históricos da mesma edição',requestBody:{required:true,content:{'application/json':{schema:{$ref:'#/components/schemas/AdmissionPossibilityRequest'}}}},responses:{200:{description:'Comparações históricas sem promessa de aprovação'},422:{$ref:'#/components/responses/Invalid'}}}},
