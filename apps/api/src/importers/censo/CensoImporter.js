...
export class CensoImporter {
  ...
  async transform() {
    ...
    await pool.query(`INSERT INTO course_catalog_records(institution_id,course_id,municipality_id,inep_course_code,original_name,dimension,degree,modality,academic_level,free_indicator,census_year,census_seats,daytime_seats,nighttime_seats,enrolled,raw_payload,snapshot_id,natural_key) SELECT i.id,c.id,m.id,${clean('CO_CURSO')},${clean('NO_CURSO')},CASE ${clean('TP_DIMENSAO')} WHEN '1' THEN 'municipio' WHEN '2' THEN 'ead_brasil' WHEN '3' THEN 'ead_brasil_agregado' WHEN '4' THEN 'ead_exterior' ELSE 'nao_confirmado' END,CASE ${clean('TP_GRAU_ACADEMICO')} WHEN '1' THEN 'bacharelado' WHEN '2' THEN 'licenciatura' WHEN '3' THEN 'tecnologo' WHEN '4' THEN 'abi' ELSE 'nao_confirmado' END,CASE ${clean('TP_MODALIDADE_ENSINO')} WHEN '1' THEN 'presencial' WHEN '2' THEN 'ead' END,CASE ${clean('TP_NIVEL_ACADEMICO')} WHEN '1' THEN 'graduacao' WHEN '2' THEN 'sequencial' END,CASE ${clean('IN_GRATUITO')} WHEN '1' THEN true WHEN '0' THEN false END,${clean('NU_ANO_CENSO')}::integer,${numeric('QT_VG_TOTAL')},${numeric('QT_VG_TOTAL_DIURNO')},${numeric('QT_VG_TOTAL_NOTURNO')},${numeric('QT_MAT')},to_jsonb(x),$1,concat($1::text,'|',i.id::text,'|',${clean('CO_CURSO')},'|',CASE ${clean('TP_DIMENSAO')} WHEN '1' THEN 'municipio' WHEN '2' THEN 'ead_brasil' WHEN '3' THEN 'ead_brasil_agregado' WHEN '4' THEN 'ead_exterior' ELSE 'nao_confirmado' END,'|',COALESCE(m.id::text,'∅'),'|',CASE ${clean('TP_GRAU_ACADEMICO')} WHEN '1' THEN 'bacharelado' WHEN '2' THEN 'licenciatura' WHEN '3' THEN 'tecnologo' WHEN '4' THEN 'abi' ELSE 'nao_confirmado' END,'|',CASE ${clean('TP_MODALIDADE_ENSINO')} WHEN '1' THEN 'presencial' WHEN '2' THEN 'ead' END,'|',COALESCE(CASE ${clean('TP_NIVEL_ACADEMICO')} WHEN '1' THEN 'graduacao' WHEN '2' THEN 'sequencial' END,'∅')) FROM stg_courses_2024 x JOIN institutions i ON i.inep_code=${clean('CO_IES')} JOIN courses c ON c.cine_code=COALESCE(${clean('CO_CINE_ROTULO')},'inep-'||${clean('CO_CURSO')}) LEFT JOIN municipalities m ON m.ibge_code=${clean('CO_MUNICIPIO')} ON CONFLICT(natural_key) DO UPDATE SET original_name=EXCLUDED.original_name,free_indicator=EXCLUDED.free_indicator,census_seats=EXCLUDED.census_seats,daytime_seats=EXCLUDED.daytime_seats,nighttime_seats=EXCLUDED.nighttime_seats,enrolled=EXCLUDED.enrolled,raw_payload=EXCLUDED.raw_payload`, [this.snapshotId]);
  }
