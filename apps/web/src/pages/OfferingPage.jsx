...
export function OfferingPage(){
  ...
  const item=query.data.data;const address=item.campus.address||{};const fullAddress=[address.street,address.number,address.neighborhood,item.location.city,item.location.state,address.postalCode].filter(Boolean).join(', ');const verifiedAt=new Date(item.updatedAt||item.source.importedAt).toLocaleDateString('pt-BR',{timeZone:'UTC'});const mapsQuery=Number.isFinite(item.campus.latitude)&&Number.isFinite(item.campus.longitude)?`${item.campus.latitude},${item.campus.longitude}`:fullAddress;
  return <><Seo title={`${item.course.name} em ${item.location.city} | Faculdade Perto`} description={`Oferta verificada de ${item.course.name} em ${item.institution.name}, com campus, modalidade e fonte oficial.`} path={`/ofertas-verificadas/${id}`}/>
    <header className="page-header"><Breadcrumbs items={[{label:item.location.state,to:`/buscar?state=${item.location.state}`},{label:item.location.city,to:`/buscar?city=${encodeURIComponent(item.location.city)}`},{label:item.course.name}]}/><p className="eyebrow"><ShieldCheck size={16}/> Curso em fonte oficial</p><h1>{item.course.name}</h1><p>{item.institution.name}{item.institution.acronym?` (${item.institution.acronym})`:''}</p></header>
    <div className="detail-grid"><section className="detail-main"><article className="info-card"><h2>Curso e unidade</h2><dl className="fact-list"><div><dt>Grau</dt><dd><BookOpenCheck size={15}/>{item.course.degree}</dd></div><div><dt>Modalidade</dt><dd>{item.course.modality}</dd></div><div><dt>Turno</dt><dd><Clock size={15}/>{item.course.shift||'Não informado'}</dd></div><div><dt>Instituição</dt><dd><Building2 size={15}/>{item.institution.name}</dd></div><div><dt>Unidade</dt><dd>{item.campus.name}</dd></div><div><dt>{item.campus.locationStatus==='confirmado'?'Endereço publicado pela instituição':'Endereço em revisão'}</dt><dd><MapPin size={15}/>{fullAddress}</dd></div></dl>{mapsQuery&&<a className="secondary-button maps-link" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(mapsQuery)}`} target="_blank" rel="noreferrer"><MapPin size={17}/> Abrir local no Google Maps</a>}</article><div className="notice"><ShieldCheck size={18}/> {item.notice} {item.campus.locationStatus==='confirmado'?'Este local tem coordenadas confirmadas pela fonte.':'Sem coordenadas verificadas, não exibimos um marcador exato de campus no mapa.'}</div></section>
