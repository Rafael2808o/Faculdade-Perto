...
export function SearchPage(){
  ...
  return <div className="search-page">
    ...
    <div className="search-toolbar"><SearchBar initialCourse={filters.q} initialCity={filters.city||filters.state}/></div>
    <div className={`search-body ${mode==='map'?'map-mode':''}`}>
      <section className="results-panel" aria-live="polite"><div className="results-meta"><h1>{query.isLoading?'Buscando…':`${pagination?.total||0} registros encontrados`}</h1><small>Censo INEP 2024 · catálogo nacional</small></div>
        <div className="filters" aria-label="Filtros"><select value={filters.network} onChange={(event)=>change('network',event.target.value)} aria-label="Categoria administrativa"><option value="">Pública e privada</option><option value="publica">Pública</option><option value="privada">Privada</option></select><select value={filters.modality} onChange={(event)=>change('modality',event.target.value)} aria-label="Modalidade"><option value="">Todas as modalidades</option><option value="presencial">Presencial</option><option value="ead">EAD</option></select><select value={filters.degree} onChange={(event)=>change('degree',event.target.value)} aria-label="Grau"><option value="">Todos os graus</option><option value="bacharelado">Bacharelado</option><option value="licenciatura">Licenciatura</option><option value="tecnologo">Tecnólogo</option></select><select value={filters.sort} onChange={(event)=>change('sort',event.target.value)} aria-label="Ordenação"><option value="relevance">Mais relevantes</option><option value="name">Nome do curso</option><option value="seats">Mais vagas em 2024</option><option value="distance" disabled={!userLocation}>Mais perto de mim</option></select></div>
