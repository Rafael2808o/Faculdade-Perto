...
export function RecordPage(){
  ...
  return <>
    ...
    <div className="detail-grid"><section className="detail-main"><div className="notice"><AlertTriangle size={18}/> {item.granularityNotice}</div>
      <article className="info-card"><h2>Dados do curso</h2><dl className="fact-list"><div><dt>Grau</dt><dd>{displayCatalogValue(item.degree.value)}</dd><DataBadge field={item.degree}/></div><div><dt>Modalidade</dt><dd>{displayCatalogValue(item.modality.value)}</dd><DataBadge field={item.modality}/></div><div><dt>Gratuidade</dt><dd>{item.free.value===true?'Gratuito':item.free.value===false?'Não gratuito':'Não confirmado'}</dd><DataBadge field={item.free}/></div><div><dt>Vagas em 2024</dt><dd>{item.censusSeats.value??'Não confirmado'}</dd><DataBadge field={item.censusSeats}/></div><div><dt>Mensalidade</dt><dd>Não confirmado</dd><DataBadge field={item.tuition}/></div><div><dt>Nota de corte</dt><dd>Não confirmado</dd><DataBadge field={item.cutoff}/></div></dl></article>
